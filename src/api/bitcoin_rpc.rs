...
const PRIORITY_RECONCILIATION_ATTEMPTS: usize = 3;
...
impl BitcoindRpc {
    ...
    async fn apply_priority_delta(
        ...
        let Err(source) = self.prioritise_transaction(&txid, delta_to_apply).await else {
            return self
                .reconcile_priority_target(transaction, target_delta)
                .await
                .map_err(|source| PriorityUpdateError::StateIndeterminate {
                    operation,
                    source,
                    details: format!(
                        "could not reconcile the fee delta to target {target_delta} after a successful mutation"
                    ),
                });
        };
    ...
    async fn reconcile_priority_target(
        &self,
        transaction: &Transaction,
        target_delta: i64,
    ) -> Result<(), BitcoindRpcError> {
        let txid = transaction.compute_txid();
        for attempt in 0..PRIORITY_RECONCILIATION_ATTEMPTS {
            let observed_delta = self.current_fee_delta(&txid).await?;
            self.sync_transaction_tracking(transaction, observed_delta);
            if observed_delta == target_delta {
                return Ok(());
            }
            if attempt + 1 == PRIORITY_RECONCILIATION_ATTEMPTS {
                return Err(BitcoindRpcError::Prioritize(format!(
                    "could not reconcile fee delta {observed_delta} to target {target_delta}"
                )));
            }

            let correction = target_delta - observed_delta;
            warn!(
                txid = %txid,
                observed_delta,
                target_delta,
                correction,
                "reconciling transaction priority to its absolute target"
            );
            if let Err(source) = self.prioritise_transaction(&txid, correction).await {
                if !source.may_have_applied_mutation() {
                    return Err(source);
                }
            }
        }
        unreachable!("priority reconciliation loop always returns")
    }
    ...
    async fn rollback_priority_delta(
        ...
        match self.prioritise_transaction(&txid, -applied_delta).await {
            Ok(()) => match self
                .reconcile_priority_target(transaction, original_delta)
                .await
            {
                ...
                Err(rollback_error) => {
                    error!(
                        ...
                        "failed to reconcile the transaction priority rollback"
                    );
                    match self.current_fee_delta(&txid).await {
...
mod tests {
    ...
    async fn mock_prioritize_bitcoind(
        ...
        match method.as_deref() {
            Some("getprioritisedtransactions") => {
                ...
                if fee_delta != 0 {
                    result.insert(state.expected_txid.clone(), json!({"fee_delta": fee_delta}));
                }
                (
                    StatusCode::OK,
                    Json(json!({
    ...
    struct FailurePathConfig {
        ...
        rollback_fails: bool,
        clear_before_negative_mutation: bool,
    }
    ...
    async fn mock_failure_path_bitcoind(
        ...
        match method.as_deref() {
            ...
            Some("prioritisetransaction") => {
                ...
                if delta < 0 && state.config.clear_before_negative_mutation {
                    state.fee_delta.store(0, Ordering::SeqCst);
                }
    ...
    async fn prioritizing_does_not_submit_a_transaction_already_in_mempool() {
        ...
        let mempool_request = received_requests
            .recv()
            .await
            .expect("getprioritisedtransactions reconciliation request");
        assert_eq!(mempool_request["method"], "getprioritisedtransactions");
    ...
    async fn prioritizing_submits_a_missing_transaction_after_applying_the_delta() {
        ...
        assert!(
            received_requests.try_recv().is_err(),
            "prioritizing a missing transaction should issue exactly five requests"
        );
    ...
    #[tokio::test]
    async fn successful_rollback_reconciles_a_delta_cleared_by_mining() {
        let (rpc, mut received_requests, state, server) =
            start_failure_path_bitcoind(FailurePathConfig {
                ...
                clear_before_negative_mutation: true,
            })
            .await;
    ...
    async fn mempool_check_failure_rolls_back_the_new_delta() {
        let (rpc, mut received_requests, state, server) =
            start_failure_path_bitcoind(FailurePathConfig {
                ...
                ambiguous_prioritize_response: false,
                rollback_fails: false,
                clear_before_negative_mutation: false,
            })
            .await;
    ...
    #[tokio::test]
    async fn successful_deprioritization_reconciles_a_delta_cleared_by_mining() {
        #[derive(Clone)]
        ...
        async fn mock_bitcoind(
            ...
            match method.as_deref() {
                Some("getprioritisedtransactions") => {
                    ...
                    let fee_delta = state.fee_delta.load(Ordering::SeqCst);
                    if fee_delta != 0 {
                        priorities.insert(state.prioritized_txid, json!({"fee_delta": fee_delta}));
                    }
                    (
                        StatusCode::OK,
                        Json(json!({
        ...
        let reconciliation_request = received_requests
            .recv()
            .await
            .expect("getprioritisedtransactions reconciliation request");
        assert_eq!(
            reconciliation_request["method"],
            "getprioritisedtransactions"
        );
        let correction_request = received_requests
            .recv()
            .await
            .expect("prioritisetransaction correction request");
        assert_eq!(correction_request["method"], "prioritisetransaction");
        assert_eq!(
            correction_request["params"],
            json!([expected_txid, 0, 100_000_000])
        );
        let reconciliation_request = received_requests
            .recv()
            .await
            .expect("final getprioritisedtransactions reconciliation request");
        assert_eq!(
            reconciliation_request["method"],
            "getprioritisedtransactions"
        );
        assert_eq!(fee_delta.load(Ordering::SeqCst), 0);
        assert!(
            received_requests.try_recv().is_err(),
